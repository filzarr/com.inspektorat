...
@section('content')
    ...
    <section class="bg-white dark:bg-gray-900 mt-5 ">
        <div class="max-w-2xl  py-10 mx-auto lg:py-16">
            ...
            <form action="/admin/vidiokegiatan/{{$popup->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="grid gap-4 mb-4 sm:grid-cols-2 sm:gap-6 sm:mb-5">
                    <div class="sm:col-span-2">
                        <label for="name" class="block mb-2 text-lg font-medium text-gray-900 dark:text-white">Judul
                            Kegiatan</label>
                        <input type="text" name="judul" id="name" value="{{$popup->judul}}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="Masukkan Judul Vidio...">
                            @error('judul')
                    ...
                    <div class="relative max-w-sm">
                        ...
                        <input datepicker datepicker-autohide type="text" name="created_at" value="{{ \Carbon\Carbon::parse($popup->created_at)->format('M/d/Y') }}"
                    ...
                    <div class="sm:col-span-2 invisible">
                        <label for="description"
                            class="block ">Deskripsi</label>
                     
                           
                          
                    </div>
                    <div class="">
                        <img src="/images/{{$popup->image}}" class="w-20 h-20" alt="">
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help"> GIF
                            (MAX. 800x400px).</p>
                            @error('image')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500 capitalize"><span class="">*{{ $message }}</p>
                            @enderror
